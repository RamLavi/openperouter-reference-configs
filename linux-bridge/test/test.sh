...
# TODO: fix ssh
{
  ...
  eval "virtctl -n $TEST_NS ssh fedora@vm/$VM1_NAME -c \"ping -I eth1 -c 3 -W 2 $VM2_IP\"" || \
    echo "  FAIL: no connectivity from [$VM1_NAME] to [$VM2_NAME][$VM2_IP]"
  echo ""
  echo ""
  echo "### Test: connectivity from vm [$VM2_NAME] to vm [$VM1_NAME][$VM1_IP]"
  eval "virtctl -n $TEST_NS ssh fedora@vm/$VM2_NAME -c \"ping -I eth1 -c 3 -W 2 $VM1_IP\"" || \
  ...
  eval "virtctl -n $TEST_NS ssh fedora@vm/$VM1_NAME -c \"ping -I eth1 -c 3 -W 2 $EXT_CLIENT_IP\"" || \
  ...
  eval "virtctl -n $TEST_NS ssh fedora@vm/$VM2_NAME -c \"ping -I eth1 -c 3 -W 2 $EXT_CLIENT_IP\"" || \
